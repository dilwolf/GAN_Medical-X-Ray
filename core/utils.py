...
#이 함수는 파이썬의 json 파일을 저장합니다.
def save_json(json_file, filename):
    with open(filename, 'w') as f:
        json.dump(json_file, f, indent=4, sort_keys=False)

# 이 함수는 주어진 신경망의 매개변수 개수를 출력합니다.
...
# 이 함수는 He 초기화를 사용하여 신경망의 가중치를 초기화합니다.
def he_init(module):
...
# 이 함수는 정규화된 이미지를 비정규화합니다.
def denormalize(x):
    out = (x + 1) / 2
    return out.clamp_(0, 1)

# 이 함수는 이미지를 디스크에 저장합니다.
def save_image(x, ncol, filename):
    x = denormalize(x)
    vutils.save_image(x.cpu(), filename, nrow=ncol, padding=0)

# 이 함수는 참조 이미지의 스타일로 원본 이미지를 번역하고 다시 복구하는 데 사용됩니다.
@torch.no_grad()
def translate_and_reconstruct(nets, args, x_src, y_src, x_ref, y_ref, filename):
    ...
    del x_concat
# 이 함수는 잠재 공간에서의 변환을 사용하여 이미지를 변환합니다.
...
# 이 함수는 참조 이미지의 스타일을 사용하여 이미지를 변환합니다.
@torch.no_grad()
...
#def translate_using_reference(nets, args, x_src, x_ref, y_ref, filename):
    ...
    #for i, s_ref in enumerate(s_ref_list):
        ...
        #individual_filename = f"{filename}_{i}.jpg"
        #save_image(x_fake_with_ref, N+1, individual_filename)
...
def translate_using_reference(nets, args, x_src, x_ref, y_ref, filename):
    ...
    for i, s_ref in enumerate(s_ref_list):
        ...
        individual_filename = f"{filename}_{i}.jpg"
        save_image(x_fake, N+1, individual_filename)
# 이 함수는 디버깅에 도움이 되는 이미지를 생성하고 저장합니다.
@torch.no_grad()
